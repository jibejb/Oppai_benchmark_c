...
"	y = 0.02 * (y - 100.0);\n"
"\n"
"	float y1 = y + 0.16*sin(t);\n"
"	float y2 = y + 0.2*sin(t);\n"
"	float y3 = y + 1.0;\n"
"	float y4 = 10.0*y + 1.2*(2.0+sin(t))*sin(t);\n"
"\n"
"	float a1 = (1.5 * exp((0.12*sin(t)-0.5) * y1*y1)) / (1.0 + exp(-20.0*(5.0*y+sin(t))));\n"

"	float a2 = ((1.5 + 0.8*y2*y2*y2) * 1.0/(1.0+exp(20.0*(5.0*y+sin(t))))) / (1.0 + exp(-(100.0*y3 + 16.0*sin(t))));\n"
"	float a3 = (0.2 * (exp(-y3*y3) + 1.0)) / (1.0 + exp(100.0*y3 + 16.0*sin(t)));\n"

"	float a4 = 0.1 / exp(2.0 * (y4*y4)*(y4*y4));\n"
"\n"
